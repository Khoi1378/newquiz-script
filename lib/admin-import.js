...
export async function getQuiz(quizId, accessCode = null) {
  return await rpc("api_get_quiz", { p_quiz_id: normQuizId(quizId), p_access_code: accessCode });
}

// progress
export async function saveProgress(quizId, stateJson) {
  const token = getToken();
  return await rpc("api_save_progress", { p_token: token, p_quiz_id: normQuizId(quizId), p_state: stateJson });
}
export async function loadProgress(quizId) {
  const token = getToken();
  return await rpc("api_get_progress", { p_token: token, p_quiz_id: normQuizId(quizId) });
}
...
export async function getLeaderboard(quizId, limit = 30) {
  return await rpc("api_get_leaderboard", {
    p_quiz_id: normQuizId(quizId),
    p_limit: limit
  });
}
...
export async function getComments(quizId) {
  return await rpc("api_get_comments", { p_quiz_id: normQuizId(quizId) });
}
export async function addComment(quizId, parentId, text) {
  const token = getToken();
  return await rpc("api_add_comment", { p_token: token, p_quiz_id: normQuizId(quizId), p_parent: parentId, p_text: text });
}
...
export async function adminCreateQuiz(payload) {
  ...
  return await rpc("admin_create_quiz", {
    p_token: token,
    p_quiz_id: normQuizId(payload.quiz_id || ""),
...
export async function adminImportVocabs(quizId, items) {
  const token = getToken();
  return await rpc("admin_import_vocabs", { p_token: token, p_quiz_id: normQuizId(quizId), p_items: items });
}
